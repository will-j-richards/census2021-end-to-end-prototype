...
gulp.task('scripts:bundle:build', () =>
...
function jekyll(opts) {
  ...
  const jekyllWatchArgs = ['--watch', '--incremental', '--drafts'];
...
gulp.task('fonts', () => {
  gulp.src('./_fonts/**/*').pipe(gulp.dest('./s/fonts/'));
});
...
gulp.task('build:jekyll', ['build:parrellel-batch'], () => {
  gulp.run('jekyll:build');
});
